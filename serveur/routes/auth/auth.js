        ...
        Employee.findOne({ email: email })
          .then((newSavedUser) => {
            if (!newSavedUser) {
              return res
                .status(422)
                .json({ error: "vérifier votre email ou votre mot de passe" });
            }
            bcrypt
              .compare(password, newSavedUser.password)
              .then((domatch) => {
                if (domatch) {
                  const detect = 2;
                  const token = jwt.sign({ _id: newSavedUser._id }, JWT_SECRET);
                  const { _id, name, email, cin, pic, timetable, client } =
                    newSavedUser;
                  MessageAdmin.findOne({ employee: newSavedUser._id }).then(
                    (resf) => {
                      const Chat = resf._id;
                      res.json({
                        detect,
                  ...
                  return res
                    .status(422)
                    .json({
                      error: "vérifier votre email ou votre mot de passe",
                    });
                }
              })
              .catch((err) => {
        ...
        bcrypt
          ...
          .then((domatch) => {
            if (domatch) {
              ...
              Message.findOne({ client: savedUser._id }).then((r) => {
...
router.post(
  ...
  (req, res) => {
    ...
    if (!name || !email || !cin || !tel || !age) {
      return res
        .status(422)
        .send({ error: "Essayer de remplir tous les champs" });
    }
    Client.findOne({ email: email })
      .then((savedUser) => {
        if (savedUser) {
          return res
            .status(422)
            .send({ error: "Il existe un autre utilisateur avec cet email" });
        }
    ...
    Employee.findOne({ email: email })
      .then((savedUser) => {
        if (savedUser) {
          return res
            .status(422)
            .json({ error: "Il existe un autre utilisateur avec ce email" });
        }
...
router.post("/mdpOublier", (req, res) => {
  crypto.randomBytes(32, (err, buffer) => {
    ...
    Client.findOne({ email: req.body.email }).then((user) => {
      if (!user) {
        Employee.findOne({ email: req.body.email }).then((employeeUser) => {
          if (!employeeUser) {
            return res
              .status(422)
              .json({ error: "Aucun utilisateur avec ce mail" });
          } else {
