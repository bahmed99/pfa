    ...
    Employee.find().then((result1) => {
      for (let i = 0; i < result1.length; i++) {
        data.push({
          ...
          cin: result1[i].cin,
          imgUrl: result1[i].pic,
          tel: result1[i].tel,
          role: "Employée",
          age: result1[i].age,
        });
      }
      Client.find().then((result2) => {
    ...
    if (err || !client) {
      Employee.findOne({ _id: req.params.id }).exec((err, employee) => {
        if (err || !employee) {
          ...
          MessageAdmin.findOne({ employee: req.params.id }).then((result1) => {
...
router.delete("/deleteEmployee/:id", requireLoginAdmin, (req, res) => {
  Employee.findOne({ _id: req.params.id })
    ...
    .then((employee) => {
      Client.find({ employee: employee.id }).then((result1) => {
        for (let i = 0; i < result1.length; i++) {
          result1[i].employee = null;
          result1[i].timetable = [];
          result1[i].save();
        }
        if (employee.pic !== "user1.png") {
          fs.unlink(
            `../client/public/uploads/profile/employes/${employee.pic}`,
            function (err) {
...
router.get("/AffectationClient", requireLoginAdmin, (req, res) => {
  Client.find({ employee: null }).then((result) => {
    res.json(result);
  });
});
...
router.put("/essai", (req, res) => {
  Client.findOne({ _id: "610b04f544b0dc31dc45ce4a" }).then((result) => {
  ...
  Client.find().then((result) => {
    ...
    for (let i = 0; i < result.length; i++) {
      for (let j = 0; j < result[i].timetable.length; j++) {
        if (
          result[i].timetable[j].start.getFullYear() ===
            new Date().getFullYear() &&
          result[i].timetable[j].start.getMonth() === new Date().getMonth() &&
          result[i].timetable[j].start.getDate() >= fd.getDate() &&
          result[i].timetable[j].start.getDate() <= ff.getDate()
        ) {
...
router.get("/nbreAvis", requireLoginAdmin, (req, res) => {
  let tab1 = [
    "Jan",
    "Fev",
    "Mar",
...
router.get("/repartitionAvis", requireLoginAdmin, (req, res) => {
  let labels = ["🤬", "🙁", "😶", "😁", "😍"];
...
router.get("/differenceAvis", requireLoginAdmin, (req, res) => {
  let labels = [
    "Jan",
    "Fev",
    "Mar",
...
router.get("/nouveauClients", requireLoginAdmin, (req, res) => {
  const data = [];
  User.find({ done: false })
    .then((resul) => {
...
router.delete("/nouveauClients/:id", requireLoginAdmin, (req, res) => {
...
router.put("/nouveauClients/:id", requireLoginAdmin, (req, res) => {
  const data = req.body.employe;
